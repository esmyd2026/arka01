    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-3">
                <Link :href="route('cooperative.drivers.index')" class="grid h-9 w-9 place-items-center rounded-full border border-arka-border text-arka-text-muted hover:text-arka-primary" aria-label="Volver">←</Link>
        ...
        <div class="mx-auto max-w-6xl space-y-5 px-4 py-6 sm:px-6">
            <section class="overflow-hidden rounded-2xl border border-arka-primary/25 bg-arka-card shadow-xl">
                ...
                <div class="flex flex-col gap-4 p-5 sm:flex-row sm:items-center">
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            <span class="rounded-full px-2.5 py-1 text-xs font-semibold" :class="membership.driver.driver_profile?.is_available ? 'bg-emerald-400/15 text-emerald-700' : 'bg-slate-400/10 text-arka-text-muted'">{{ membership.driver.driver_profile?.is_available ? 'Disponible ahora' : 'Desconectado' }}</span>
                        </div>
            ...
            <section class="grid gap-5 lg:grid-cols-[1.25fr_.75fr]">
                <article class="rounded-2xl bg-arka-card p-5">
                    ...
                    <div class="mt-5 grid grid-cols-3 gap-3 text-center">
                        ...
                        <div class="rounded-xl bg-black/10 p-3"><p class="text-xl font-bold text-emerald-700">{{ summary.completed_total }}</p><p class="text-xs text-arka-text-muted">Completadas</p></div>
            ...
            <section v-if="summary.wallet_balance !== 0" class="rounded-2xl p-5" :class="summary.wallet_balance > 0 ? 'bg-amber-400/10' : 'bg-emerald-400/10'">
                <p class="text-xs uppercase tracking-widest" :class="summary.wallet_balance > 0 ? 'text-amber-700' : 'text-emerald-700'">Billetera</p>
            ...
            <section class="rounded-2xl bg-arka-card">
                <div class="border-b border-arka-border p-5"><p class="text-xs uppercase tracking-widest text-arka-primary">Historial</p><h2 class="mt-1 font-semibold text-arka-text">Carreras e ingresos individuales</h2></div>
                ...
                <div v-if="rides.links.length > 3" class="flex flex-wrap gap-2 border-t border-arka-border p-4"><Link v-for="link in rides.links" :key="link.label" :href="link.url || '#'" v-html="link.label" class="rounded-lg px-3 py-1.5 text-xs" :class="link.active ? 'bg-arka-primary text-black' : 'bg-black/10 text-arka-text-muted'" /></div>
            </section>
        </div>
    </AuthenticatedLayout>
</template>
