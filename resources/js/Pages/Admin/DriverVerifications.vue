    ...
    <AdminLayout title="Verificación y activación de conductores">
        <div class="py-8 sm:py-12">
            <div class="mx-auto max-w-5xl space-y-6 px-4 sm:px-6 lg:px-8">
                <section class="rounded-arka border border-arka-primary/30 bg-arka-card p-4 shadow sm:p-6">
                    ...
                    <div class="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
                        ...
                        <div class="rounded-xl border border-arka-primary/30 bg-arka-primary/5 p-3"><p class="text-2xl font-bold text-arka-primary-bright">{{ approved.length }}</p><p class="text-xs font-semibold text-arka-text">4. Activados recientes</p><p class="mt-1 text-[11px] text-arka-text-muted">Ya pueden conectarse.</p></div>
                    </div>
                </section>

                <section class="rounded-arka border border-arka-warning/25 bg-arka-card shadow">
                    <header class="flex items-center justify-between gap-3 border-b border-arka-border p-4 sm:p-5">
                    ...
                    <ul v-else class="divide-y divide-arka-border">
                        <li v-for="profile in pending" :key="profile.id" class="space-y-4 p-4 sm:p-6">
                            ...
                            <div class="grid gap-3 lg:grid-cols-[minmax(0,1.45fr)_minmax(17rem,0.55fr)]">
                                <section class="min-w-0 rounded-xl border border-arka-border bg-arka-base/50 p-3">
                                    ...
                                    <div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
                                        <button v-for="document in documentsFor(profile)" :key="document.key" type="button" class="group min-w-0 overflow-hidden rounded-xl border border-arka-border bg-arka-card text-left hover:border-arka-primary/40" @click="viewingDocument = document">
                                ...
                                <aside class="rounded-xl border border-arka-border bg-arka-base/50 p-3">
                            ...
                            <div class="rounded-xl border border-arka-primary/25 bg-arka-base p-3 sm:p-4">
                ...
                <details class="rounded-arka border border-arka-border bg-arka-card shadow">
                    ...
                    <ul class="divide-y divide-arka-border border-t border-arka-border"><li v-for="driver in registered" :key="driver.id" class="flex items-center justify-between gap-3 p-4"><div><p class="font-medium text-arka-text">{{ driver.full_name || driver.name }}</p><p class="text-xs text-arka-text-muted">{{ driver.email }} · Registrado {{ formatDate(driver.created_at) }}</p></div><Link :href="route('admin.users.show', driver.id)" class="text-xs font-medium text-arka-primary">Ver cuenta</Link></li><li v-if="!registered.length" class="p-4 text-sm text-arka-text-muted">No hay registros detenidos en esta etapa.</li></ul>
                </details>
                ...
                <details class="rounded-arka border border-arka-border bg-arka-card shadow">
                    ...
                    <ul class="divide-y divide-arka-border border-t border-arka-border"><li v-for="profile in incomplete" :key="profile.id" class="flex items-center justify-between gap-3 p-4"><div><p class="font-medium text-arka-text">{{ profile.user.full_name || profile.user.name }}</p><p class="text-xs text-arka-text-muted">Actualizado {{ formatDate(profile.updated_at) }} · Faltan datos o documentos</p></div><Link :href="route('admin.users.show', profile.user.id)" class="text-xs font-medium text-arka-primary">Revisar avance</Link></li><li v-if="!incomplete.length" class="p-4 text-sm text-arka-text-muted">No hay expedientes incompletos.</li></ul>
                </details>
                ...
                <details class="rounded-arka border border-arka-danger/15 bg-arka-card shadow">
                    ...
                    <ul class="divide-y divide-arka-border border-t border-arka-border"><li v-for="profile in rejected" :key="profile.id" class="p-4"><div class="flex items-center justify-between gap-3"><p class="font-medium text-arka-text">{{ profile.user.full_name || profile.user.name }}</p><Link :href="route('admin.users.show', profile.user.id)" class="text-xs font-medium text-arka-primary">Ver expediente</Link></div><p class="mt-1 text-xs text-arka-danger">{{ profile.verification_rejection_reason || 'Sin motivo registrado' }}</p></li><li v-if="!rejected.length" class="p-4 text-sm text-arka-text-muted">No hay correcciones pendientes.</li></ul>
                </details>

                <details class="rounded-arka border border-arka-primary/25 bg-arka-card shadow">
                    ...
                    <ul class="divide-y divide-arka-border border-t border-arka-border"><li v-for="profile in approved" :key="profile.id" class="flex items-center justify-between gap-3 p-4"><div><p class="font-medium text-arka-text">{{ profile.user.full_name || profile.user.name }}</p><p class="text-xs text-arka-text-muted">Activado {{ formatDate(profile.verified_at) }}</p></div><DriverCategoryBadge :label="profile.public_category ? publicDriverCategories[profile.public_category]?.label : null" /></li><li v-if="!approved.length" class="p-4 text-sm text-arka-text-muted">Todavía no hay conductores activados.</li></ul>
                </details>
            </div>
        </div>
        ...
        <Modal :show="viewingDocument !== null" max-width="2xl" @close="viewingDocument = null">
            <div v-if="viewingDocument" class="p-4 sm:p-5">
                <div class="mb-3 flex items-center justify-between gap-3"><div><p class="text-xs font-bold uppercase tracking-[0.12em] text-arka-primary-bright">Documento del conductor</p><h3 class="mt-1 text-lg font-semibold text-arka-text">{{ viewingDocument.label }}</h3></div><button type="button" class="grid h-9 w-9 place-items-center rounded-full border border-arka-border text-arka-text-muted hover:text-arka-text" aria-label="Cerrar visor" @click="viewingDocument = null">×</button></div>
                <div class="grid h-[min(68vh,720px)] place-items-center overflow-hidden rounded-xl border border-arka-border bg-black/30">
