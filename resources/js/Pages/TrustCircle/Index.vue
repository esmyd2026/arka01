    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between gap-3">
                ...
                <button type="button" class="inline-flex min-h-10 items-center gap-2 rounded-xl bg-arka-primary px-4 text-xs font-bold text-white" @click="activeTab = 'people'; showSearch = !showSearch">
        ...
        <main class="mx-auto max-w-3xl space-y-4 px-3 py-4 pb-24 sm:px-6 sm:py-6">
            <section class="rounded-2xl border border-arka-border bg-arka-card p-4 sm:p-5">
                <div class="flex items-center gap-4">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <div class="profile-stats mt-4 grid grid-cols-3 divide-x divide-arka-border text-center">
            ...
            <section v-if="people.length" aria-label="Personas de tu círculo" class="overflow-hidden rounded-2xl border border-arka-border bg-arka-card py-3">
            ...
            <nav class="grid rounded-2xl border border-arka-border bg-arka-card p-1" :class="canBuildFleet ? 'grid-cols-3' : 'grid-cols-2'" aria-label="Secciones del círculo">
            ...
            <section v-if="activeTab === 'people'" class="space-y-3">
                <div v-if="showSearch" class="rounded-2xl border border-arka-primary/30 bg-arka-card p-4">
                    ...
                    <div class="relative mt-3"><svg class="pointer-events-none absolute left-3 top-3.5 h-5 w-5 fill-none stroke-arka-text-muted" viewBox="0 0 24 24"><circle cx="11" cy="11" r="7" stroke-width="1.7"/><path d="m16 16 4 4" stroke-width="1.7" stroke-linecap="round"/></svg><input v-model="search" autofocus class="w-full rounded-xl border border-arka-border bg-arka-base py-3 pl-10 pr-20 text-sm text-arka-text placeholder:text-arka-text-muted/70 focus:border-arka-primary focus:ring-arka-primary" placeholder="Nombre, @usuario o código"><span v-if="searching" class="absolute right-3 top-3.5 text-xs text-arka-primary">Buscando…</span></div>
                    <p v-if="searchError" class="mt-2 text-xs text-red-700">{{ searchError }}</p>
                    <div v-if="searchResults.length" class="mt-3 divide-y divide-arka-border"><article v-for="person in searchResults" :key="person.user_public_id" class="flex items-center gap-3 py-3"><UserAvatar :user="person" size-class="h-10 w-10 text-sm shrink-0" /><div class="min-w-0 flex-1"><strong class="block truncate text-sm text-arka-text">{{ person.name }}</strong><span class="text-xs text-arka-text-muted">@{{ person.username }} · {{ person.role }}</span></div><span v-if="person.connection_status" class="text-xs text-arka-text-muted">{{ person.connection_status === 'accepted' ? 'Conectado' : 'Pendiente' }}</span><button v-else class="rounded-lg bg-arka-primary px-3 py-2 text-xs font-bold text-white" @click="sendRequest(person)">Conectar</button></article></div>
                </div>
                ...
                <article v-for="person in people" :key="person.connection_public_id" class="overflow-hidden rounded-2xl border border-arka-border bg-arka-card">
                    ...
                    <div v-if="openPerson === person.connection_public_id" class="border-t border-arka-border p-4">
                        ...
                        <div class="mt-4 rounded-xl bg-arka-base/45 p-3"><div class="flex items-center justify-between gap-3"><div><strong class="block text-xs text-arka-text">Compartir mi índice</strong><small class="text-[11px] text-arka-text-muted">Muestra reputación y actividad.</small></div><input v-model="privacy[person.connection_public_id].share_rating" type="checkbox" class="rounded border-arka-text-muted/30 bg-arka-base text-arka-primary focus:ring-arka-primary"></div><div v-if="canBuildFleet" class="mt-3 flex items-center justify-between gap-3"><div><strong class="block text-xs text-arka-text">Compartir mis conductores</strong><small class="text-[11px] text-arka-text-muted">Podrá descubrirlos y pedir invitarlos.</small></div><input v-model="privacy[person.connection_public_id].share_fleet" type="checkbox" class="rounded border-arka-text-muted/30 bg-arka-base text-arka-primary focus:ring-arka-primary"></div><label class="mt-3 block text-xs font-semibold text-arka-text">Cómo la conoces<input v-model="privacy[person.connection_public_id].relationship_label" class="mt-1.5 w-full rounded-lg border border-arka-border bg-arka-base px-3 py-2 text-sm text-arka-text" placeholder="Familia, amistad, trabajo…"></label></div>
                        <div class="mt-3 flex items-center justify-between gap-3"><button class="rounded-lg bg-arka-primary px-3 py-2 text-xs font-bold text-white" @click="savePrivacy(person)">Guardar</button><button class="text-xs text-red-700" @click="removeConnection(person)">Quitar del círculo</button></div>
                    </div>
                </article>

                <div v-if="!people.length" class="rounded-2xl border border-dashed border-arka-border bg-arka-card p-8 text-center"><span class="mx-auto grid h-12 w-12 place-items-center rounded-full bg-arka-primary/10 text-arka-primary"><svg class="h-6 w-6 fill-none stroke-current" viewBox="0 0 24 24"><path d="M8 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8Zm8-1a3.5 3.5 0 1 0 0-7M2 21a6 6 0 0 1 12 0m1-7a5 5 0 0 1 7 4.6" stroke-width="1.7" stroke-linecap="round" /></svg></span><strong class="mt-3 block text-sm text-arka-text">Tu círculo empieza con una persona</strong><p class="mt-1 text-xs text-arka-text-muted">Agrega a alguien que realmente conoces.</p></div>

                <details class="help-panel overflow-hidden rounded-2xl border border-arka-border bg-arka-card"><summary class="flex cursor-pointer list-none items-center justify-between gap-3 p-4"><div><strong class="text-sm text-arka-text">¿Cómo Funciona?</strong><p class="mt-0.5 text-xs text-arka-text-muted">Una guía corta sobre conexiones y privacidad.</p></div><svg class="help-chevron h-5 w-5 fill-none stroke-arka-text-muted transition-transform" viewBox="0 0 24 24"><path d="m7 9 5 5 5-5" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" /></svg></summary><div class="border-t border-arka-border p-4"><div class="grid gap-3 sm:grid-cols-2"><div class="flow-step"><span>1</span><p><strong>Busca a alguien</strong> por nombre, usuario o código.</p></div><div class="flow-step"><span>2</span><p><strong>Debe aceptar.</strong> Nadie entra automáticamente.</p></div><div class="flow-step"><span>3</span><p><strong>Descubre coincidencias</strong> y conductores compartidos.</p></div><div class="flow-step"><span>4</span><p><strong>Tú controlas</strong> el índice y la flota que compartes.</p></div></div><p class="mt-3 text-[11px] leading-5 text-arka-text-muted">El índice resume señales internas de Arka01. No certifica identidad ni garantiza seguridad.</p></div></details>
            </section>

            <section v-else-if="activeTab === 'requests'" class="space-y-3">
                <div v-if="receivedRequests.length"><div class="mb-3 flex items-center justify-between"><h3 class="text-sm font-semibold text-arka-text">Quieren conectar contigo</h3><span class="rounded-full bg-arka-primary/10 px-2 py-1 text-[10px] font-bold text-arka-primary">{{ receivedRequests.length }} nuevas</span></div><article v-for="request in receivedRequests" :key="request.connection_public_id" class="rounded-2xl border border-arka-primary/25 bg-arka-card p-4"><div class="flex items-start gap-3"><UserAvatar :user="request" size-class="h-12 w-12 text-base shrink-0" /><div class="min-w-0 flex-1"><strong class="block truncate text-sm text-arka-text">{{ request.name }}</strong><span class="text-xs text-arka-text-muted">@{{ request.username }} · Socio #{{ request.member_code }}</span><div class="mt-2 flex flex-wrap gap-1.5"><span class="rounded-full bg-arka-surface px-2 py-0.5 text-[10px] text-arka-text-muted">{{ request.role }}</span><span v-if="request.relationship_label" class="rounded-full bg-arka-surface px-2 py-0.5 text-[10px] text-arka-text-muted">Te conoce como {{ request.relationship_label }}</span><TrustScoreBadge :trust="request.trust" compact /></div></div></div><div class="mt-4 grid grid-cols-3 gap-2"><Link :href="route('profiles.show', request.user_public_id)" class="grid min-h-10 place-items-center rounded-xl border border-arka-primary/25 text-xs font-semibold text-arka-primary">Ver perfil</Link><button class="min-h-10 rounded-xl bg-arka-primary text-xs font-bold text-white" @click="respond(request, 'accept')">Aceptar</button><button class="min-h-10 rounded-xl bg-arka-surface text-xs font-semibold text-arka-text-muted" @click="respond(request, 'reject')">Ahora no</button></div></article></div>
                <div v-if="sentRequests.length"><h3 class="mb-3 text-sm font-semibold text-arka-text">Solicitudes enviadas</h3><div class="divide-y divide-arka-border rounded-2xl border border-arka-border bg-arka-card px-4"><div v-for="request in sentRequests" :key="request.connection_public_id" class="flex items-center gap-3 py-3"><UserAvatar :user="request" size-class="h-10 w-10 text-sm shrink-0" /><div class="min-w-0 flex-1"><strong class="block truncate text-sm text-arka-text">{{ request.name }}</strong><span class="text-xs text-arka-text-muted">@{{ request.username }}</span></div><span class="rounded-full bg-arka-surface px-2 py-1 text-[10px] text-arka-text-muted">Pendiente</span></div></div></div>
                <div v-if="!requestCount" class="rounded-2xl border border-dashed border-arka-border bg-arka-card p-8 text-center"><strong class="text-sm text-arka-text">No tienes solicitudes pendientes</strong><p class="mt-1 text-xs text-arka-text-muted">Cuando alguien quiera conectar, aparecerá aquí.</p><button class="mt-4 rounded-xl bg-arka-primary px-4 py-2.5 text-xs font-bold text-white" @click="activeTab = 'people'; showSearch = true">Buscar personas</button></div>
            </section>

            <section v-else class="space-y-3">
                <div class="rounded-2xl border border-arka-border bg-arka-card p-4"><h3 class="text-sm font-semibold text-arka-text">Conductores que conoce tu círculo</h3><p class="mt-1 text-xs leading-5 text-arka-text-muted">Solo aparecen desde flotas que sus dueños decidieron compartir contigo.</p></div>
                <article v-for="driver in recommendedDrivers" :key="driver.driver_public_id" class="rounded-2xl border border-arka-border bg-arka-card p-4"><div class="flex items-center gap-3"><UserAvatar :user="driver" size-class="h-12 w-12 text-base shrink-0" /><div class="min-w-0 flex-1"><strong class="block truncate text-sm text-arka-text">{{ driver.name }}</strong><span class="text-xs text-arka-text-muted">Socio #{{ driver.member_code }}</span><p class="mt-1 text-xs text-arka-primary">Lo conocen {{ driver.recommended_by_count }} persona{{ driver.recommended_by_count === 1 ? '' : 's' }} de tu círculo</p></div><TrustScoreBadge :trust="driver.trust" compact /></div><p v-if="driver.recommended_by.length" class="mt-3 text-xs text-arka-text-muted">Conocido por {{ driver.recommended_by.join(', ') }}.</p><div class="mt-4 grid grid-cols-2 gap-2"><Link :href="route('profiles.show', driver.driver_public_id)" class="grid min-h-10 place-items-center rounded-xl border border-arka-primary/25 text-xs font-semibold text-arka-primary">Ver perfil</Link><button class="min-h-10 rounded-xl bg-arka-primary text-xs font-bold text-white" @click="inviteDriver(driver)">Invitar a mi flota</button></div></article>
                <div v-if="!recommendedDrivers.length" class="rounded-2xl border border-dashed border-arka-border bg-arka-card p-8 text-center"><strong class="text-sm text-arka-text">Todavía no hay conductores compartidos</strong><p class="mt-1 text-xs leading-5 text-arka-text-muted">Cuando alguien de tu círculo comparta su flota, sus conductores aparecerán aquí.</p></div>
            </section>
        </main>
    </AuthenticatedLayout>
</template>
...
<style scoped>
.score-mini { width: 46px; height: 46px; flex: 0 0 auto; display: grid; place-items: center; border-radius: 50%; background: conic-gradient(rgb(var(--arka-primary)) var(--score), rgb(var(--arka-border) / .55) 0); position: relative; box-shadow: 0 0 0 1px rgb(var(--arka-border) / .45); }
.score-mini::before { content: ''; position: absolute; inset: 4px; border-radius: inherit; background: rgb(var(--arka-card)); }
.score-mini span { position: relative; z-index: 1; color: rgb(var(--arka-text)); font-size: .78rem; font-weight: 800; }
.profile-stats strong { display: block; color: rgb(var(--arka-text)); font-size: .9rem; }.profile-stats span { display: block; margin-top: .12rem; color: rgb(var(--arka-text-muted)); font-size: .58rem; }
.circle-strip { scrollbar-width: none; }.circle-strip::-webkit-scrollbar { display: none; }
.social-tab { min-height: 3rem; position: relative; display: flex; align-items: center; justify-content: center; gap: .4rem; border-radius: .8rem; color: rgb(var(--arka-text-muted)); font-size: .7rem; font-weight: 700; transition: .18s ease; }
...
.social-tab.active { background: rgb(var(--arka-primary) / .12); color: rgb(var(--arka-primary-bright)); }.social-tab i { min-width: 1rem; height: 1rem; display: grid; place-items: center; border-radius: 999px; background: rgb(var(--arka-primary)); color: rgb(var(--arka-on-primary)); font-size: .55rem; font-style: normal; }
.social-tab:focus-visible { outline: 2px solid rgb(var(--arka-primary)); outline-offset: 1px; }
.help-panel[open] .help-chevron { transform: rotate(180deg); }
.flow-step { display: flex; align-items: flex-start; gap: .65rem; border-radius: .75rem; padding: .7rem; background: rgb(var(--arka-surface)); color: rgb(var(--arka-text-muted)); font-size: .7rem; line-height: 1.5; }.flow-step > span { width: 1.5rem; height: 1.5rem; flex: 0 0 auto; display: grid; place-items: center; border-radius: 50%; background: rgb(var(--arka-primary) / .12); color: rgb(var(--arka-primary-bright)); font-size: .65rem; font-weight: 800; }.flow-step strong { color: rgb(var(--arka-text)); }
</style>
