...
<template>
    <Transition name="route-loader-fade">
        <div
            v-if="show"
            class="fixed inset-0 z-[1800] grid place-items-center bg-arka-ink/25 px-6 backdrop-blur-[6px]"
            role="status"
            aria-live="polite"
            aria-busy="true"
        >
            <div class="w-full max-w-[310px] rounded-[28px] border border-arka-border bg-arka-card px-6 py-7 text-center shadow-[0_28px_90px_rgba(4,20,13,0.28)]">
                ...
                <h2 class="mt-5 text-lg font-bold text-arka-text">{{ title }}</h2>
...
.arka-route-loader__ring { position: absolute; inset: 2px; border-radius: 999px; border: 2px solid rgb(var(--arka-primary) / .16); border-top-color: rgb(var(--arka-primary)); border-right-color: rgb(var(--arka-lime) / .7); animation: arka-route-spin 1.15s linear infinite; }
.arka-route-loader__ring--inner { inset: 12px; border-width: 1px; border-color: rgb(var(--arka-border) / .7); border-bottom-color: rgb(var(--arka-primary) / .8); animation-duration: 1.8s; animation-direction: reverse; }
.arka-route-loader__core { width: 72px; height: 72px; position: relative; z-index: 2; display: grid; place-items: center; overflow: hidden; border-radius: 999px; background: radial-gradient(circle at 35% 25%, rgb(var(--arka-card)) 0%, rgb(var(--arka-surface)) 70%); box-shadow: inset 0 0 0 1px rgb(var(--arka-border) / .75), 0 12px 32px rgb(var(--arka-primary) / .2); }
:global(html.dark) .arka-route-loader__core { background: radial-gradient(circle at 35% 25%, rgb(var(--arka-primary) / .22), rgb(var(--arka-surface)) 72%); }
...
.arka-loading-pulse { width: 6px; height: 6px; border-radius: 999px; background: rgb(var(--arka-primary)); animation: arka-route-pulse 1.15s ease-in-out infinite; }.arka-loading-pulse:nth-child(2) { animation-delay: .16s; }.arka-loading-pulse:nth-child(3) { animation-delay: .32s; }
