...
<template>
    <Transition name="route-loader-fade">
        <div
            ...
            aria-busy="true"
        >
            <div class="w-full max-w-[310px] rounded-[28px] border border-arka-border bg-white px-6 py-7 text-center shadow-[0_28px_90px_rgba(23,34,29,0.20)]">
...
.arka-route-loader__ring { position: absolute; inset: 2px; border-radius: 999px; border: 2px solid rgba(20,125,88,.14); border-top-color: #147d58; border-right-color: rgba(101,163,13,.65); animation: arka-route-spin 1.15s linear infinite; }
.arka-route-loader__ring--inner { inset: 12px; border-width: 1px; border-color: #d9e2dd; border-bottom-color: rgba(20,125,88,.75); animation-duration: 1.8s; animation-direction: reverse; }
...
.arka-loading-pulse { width: 6px; height: 6px; border-radius: 999px; background: #147d58; animation: arka-route-pulse 1.15s ease-in-out infinite; }.arka-loading-pulse:nth-child(2) { animation-delay: .16s; }.arka-loading-pulse:nth-child(3) { animation-delay: .32s; }
