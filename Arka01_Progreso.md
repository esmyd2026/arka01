...
### Teléfono en el registro, recordatorio al conductor y chat pre-viaje

Pedido explícito del usuario, con tres capturas: el campo de teléfono dejaba escribir de más (una captura con 27 dígitos), pidió una alarma para el conductor 15-20 min antes de una carrera programada, y un chat entre conductor y cliente para confirmar fecha/hora de recogida. También preguntó si ocultar o dejar como "más información" el aviso nuevo de ubicación del registro (pasada anterior).

- **Bug real: el teléfono no tenía límite de verdad**. `Auth/Register.vue` invalidaba el paso al mandar el formulario (el botón "Siguiente" ya quedaba deshabilitado), pero nada impedía seguir tecleando de más mientras tanto. Ahora se sanea en cada tecla: saca todo lo que no sea dígito, el 0 inicial si es Ecuador (ya lo reemplaza el código de país) y corta al máximo real del país elegido (9 para Ecuador, 10 para el resto) — con `maxlength` nativo de respaldo. Si cambia de país con un número ya escrito, se vuelve a sanear contra el nuevo máximo.
- **Nota de ubicación → colapsable**: respondiendo la pregunta del usuario ("¿ocultá esto o dejalo como más información?") — se combinaron las dos: queda oculta por defecto, con un `<details>` "📍 ¿Por qué nos pide la ubicación?" que la muestra si alguien quiere leerla, en vez de ocupar espacio siempre.
- **Recordatorio 15-20 min antes de una carrera programada** (pedido explícito del usuario, "alarma para el conductor"): `rides.driver_reminder_sent_at` (nueva, bandera para no avisar dos veces) + `App\Console\Commands\SendUpcomingRideReminders`, corriendo cada 5 minutos (`Kernel::schedule`, mismo patrón que `drivers:sweep-stale-availability`) — barre las carreras `scheduled` cuya hora cae en la ventana de 15-20 min desde ahora y le manda al conductor push (`RideScheduledReminderPushNotification`) + aviso en vivo si tiene la app abierta (`RideReminderDue`, mismo criterio de canales que `RideStarted`/`RideCancelled` — canal de flota + personal). Solo al conductor, como se pidió.
- **Chat entre conductor y cliente para una carrera programada**: ya existía de punta a punta (`Ride::chatIsOpen()`/`chatOpen` en `Ride/Show.vue` ya cubrían `status === 'scheduled'`, no solo `in_progress`) — lo que faltaba era que sirviera para ANTES del día: las "respuestas rápidas" eran todas del tipo "voy en camino"/"ya llegué", sin sentido para una carrera que puede faltar días. Se agregó un segundo set (`QUICK_REPLIES_*_SCHEDULED`) que se muestra en cambio mientras `ride.status === 'scheduled'`: confirmar la hora, avisar una demora del día, pedir cambiar el horario.
- **Bug propio detectado de paso, relacionado**: desde el Dashboard, la tarjeta de cada carrera en "Próximos viajes" enlazaba siempre a la lista general (`rides.index`), nunca directo al detalle/chat de ESA carrera puntual — dos clics para llegar al chat en vez de uno. `DashboardController::upcomingTripsFor()` ya traía `ride_id` para las confirmadas/programadas, solo no se usaba; ahora el link va directo a `rides.show` cuando existe.

### Tests
`tests/Feature/Ride/SendUpcomingRideRemindersTest.php` (nuevo, 4 tests): manda el recordatorio dentro de la ventana de 15-20 min, no lo repite si ya se mandó, ignora carreras fuera de la ventana (muy pronto o muy lejos) y carreras que ya arrancaron. Sin tests nuevos de PHP para el saneo del teléfono ni las respuestas rápidas (cambios puramente de Vue, sin lógica de backend). Suite completa: 672 tests OK, Pint limpio, build limpio.

**Pendiente de tu parte**: la migración nueva (`driver_reminder_sent_at`) tampoco se pudo correr en la base local — mismo MySQL caído, corré `php artisan migrate` apenas lo levantes. El recordatorio corre desde el scheduler de Laravel (`php artisan schedule:work`, ya mencionado como proceso opcional al principio de este documento) — sin eso corriendo en local, el aviso de 15-20 min no se dispara solo (podés probarlo a mano con `php artisan rides:send-upcoming-reminders`).

---

### Pantalla de carga inicial y el conductor puede cancelar (con motivo)

Pedido explícito del usuario, con captura: una pantalla de carga al arrancar la app (fondo negro, logo, sonido de bocina de barco antiguo), y que el conductor también pueda cancelar una carrera — con un motivo y una observación opcional.

- **`Components/SplashScreen.vue`** (nuevo): overlay fijo negro con el logo centrado, montado una sola vez en `app.js` (junto a `ConfirmDialogHost`) — no en cada layout, así solo aparece en el arranque real de la app (F5 o primera visita), nunca en una navegación de Inertia normal (esas no recargan la página). Se desvanece solo a los 1.8 s.
- **`playShipHorn()`** (nuevo, `Utils/liveAlert.js`): mismo criterio que el resto de los avisos sonoros del proyecto — se sintetiza con Web Audio API (dos osciladores graves apenas desafinados entre sí para el "beating" grueso de una bocina real), nada de archivo de audio que alojar. **Ojo**: los navegadores bloquean el autoplay de sonido sin una interacción previa del usuario en la pestaña — puede no sonar en la primera carga en frío según el navegador (política de autoplay, no un bug de acá), sin que eso rompa la pantalla.
- **El conductor ahora también puede cancelar una carrera** (antes solo el cliente) — `RideController::cancel()` acepta a cualquiera de las dos partes, exige un `reason` (lista fija por rol, `CLIENT_CANCEL_REASONS`/`DRIVER_CANCEL_REASONS` — no un catálogo administrable como `RatingReason`, alcanza con código) y una `note` libre, opcional. Nuevas columnas `rides.cancelled_by`/`cancellation_reason`/`cancellation_note`. El aviso (push + WebSocket) le llega a quien NO canceló, nunca a quien mandó la acción — `RideCancelledPushNotification` ahora dice quién fue y por qué.
- **`Ride/Show.vue`**: el botón "Cancelar carrera" (ahora visible para las dos partes) abre un formulario en el lugar — motivo (`<select>`, opciones según el rol) + observación opcional — en vez del simple diálogo de confirmar de antes. La tarjeta de "carrera cancelada" ahora también muestra quién canceló, el motivo y la observación si la dejó.

### Tests
`tests/Feature/Ride/RideRequestFlowTest.php` (+5, tests existentes de cancelar actualizados a mandar `reason`): el conductor puede cancelar con motivo y observación opcional (le llega el aviso al cliente, no al conductor), cancelar sin observación es válido, cancelar sin motivo se rechaza, un motivo de la lista del otro rol se rechaza, un desconocido no puede cancelar (reemplaza al viejo test de "el conductor no puede cancelar", que ya no aplica). Nada de PHP para la pantalla de carga (puramente visual/Vue). Suite completa: 676 tests OK, Pint limpio, build limpio.

**Pendiente de tu parte**: las migraciones nuevas (`cancelled_by`/`cancellation_reason`/`cancellation_note`) tampoco se pudieron correr en la base local — mismo MySQL caído, corré `php artisan migrate` apenas lo levantes.

---

### Reglas de negocio al programar, observación del cliente y mapa más chico

Pedido explícito del usuario, con captura del modal "¡Carrera nueva!" sin hora: la validación de "en el futuro" al programar no alcanzaba (dejaba programar para dentro de minutos, y no evitaba pisarle el horario a un conductor que ya tenía otro compromiso), faltaba mostrar la hora programada en el modal del conductor, faltaba un campo de observación opcional al programar, y el mapa de "Pedir carrera" se veía demasiado grande.

- **Anticipación mínima de 2 horas** (`RideRequestController::MIN_SCHEDULING_LEAD_HOURS`): reemplaza el viejo chequeo de "en el futuro" — aplica también programando para el mismo día, que es justo el caso que el usuario señaló.
- **Conflicto de horario del conductor** (`SCHEDULED_CONFLICT_BUFFER_MINUTES = 60`): si la solicitud programada va dirigida a un conductor puntual, se valida que no tenga ya otra carrera **aceptada** (`rides.status = 'scheduled'`, un compromiso real, no una solicitud que todavía nadie tomó) dentro de una ventana de una hora antes/después del horario pedido — no se conoce cuánto dura cada carrera, así que cada una bloquea ese bloque de tiempo a su alrededor.
- **Bug real: el modal "¡Carrera nueva!" decía "📅 Programada" sin ninguna hora** — el dato ya viajaba en el evento (`RideRequested::broadcastWith()`), el modal (`IncomingRideRequestModal.vue`) simplemente no lo mostraba en el template (a diferencia de la lista "Solicitudes para usted", que sí lo hacía). Ahora dice "Programada para [fecha y hora]", mismo formato que el resto de la app.
- **Observación opcional del cliente** (`ride_requests.notes`/`rides.notes`, nuevas): campo de texto libre al programar una carrera (ej. "el portón es el azul"), nunca obligatorio. Se copia a la carrera al aceptar (mismo criterio que `payment_method`) para que el conductor la siga teniendo a mano durante todo el viaje — visible en el modal de carrera entrante, en la lista de solicitudes, y en el detalle de la carrera.
- **Mapa más chico en "Pedir carrera"** (pedido explícito del usuario: "se ve muy extenso"): arranca minimizado (160px en vez de 320px) con un botón "Expandir"/"Minimizar" — sigue siendo clickeable para marcar el destino en los dos estados, solo cambia el alto.

### Tests
`tests/Feature/Ride/RideRequestFlowTest.php` (+8): programar con menos de 2 horas de anticipación se rechaza, exactamente 2 horas se acepta, un conductor puntual con otra carrera aceptada cerca de ese horario se rechaza, uno con un hueco de sobra se acepta, la observación es opcional (se guarda si viene, queda `null` si no), y se copia a la carrera al aceptar. Nada de PHP para el modal/mapa (cambios puramente de Vue). Suite completa: 683 tests OK, Pint limpio, build limpio.

**Pendiente de tu parte**: las migraciones nuevas (`ride_requests.notes`/`rides.notes`) tampoco se pudieron correr en la base local — mismo MySQL caído, corré `php artisan migrate` apenas lo levantes.

---

### Sonido atado al consentimiento, bug de notificaciones, y varios pendientes de perfil

Pedido explícito del usuario, con varias cosas juntas: que el sonido dependa del mismo consentimiento que activar notificaciones; reportó que el aviso de "Active ubicación y notificaciones" no desaparecía después de activarlas; pidió los botones "Instalar app" y "Pasarme a..." también en la pantalla de perfil; que el cliente pueda verificar su teléfono; un bug real de cuentas de Google sin poder cambiar contraseña; y mostrar el usuario/código de socio en el perfil. De paso, no le gustó el sonido de bocina de barco de la pasada anterior ("suena como que hay un error") y pidió algo más sutil.

- **Bug real: el aviso de notificaciones quedaba pegado en pantalla.** Causa: si `pushManager.subscribe()` o el POST que guarda la suscripción fallaban por cualquier motivo (VAPID mal configurado, ya suscripto con otra llave, etc.), la excepción sin atrapar cortaba `subscribeToPush()` (`push.js`) a mitad de camino — quien llamaba (`PermissionsPrompt.vue`) nunca llegaba a refrescar el estado, así que el aviso se quedaba ahí aunque el navegador ya hubiera concedido el permiso de verdad. Ahora `subscribeToPush()` atrapa cualquier error y devuelve `false` en vez de tirarlo, y `enableNotifications()` refresca el estado en un `finally` (nunca depende de que nada de arriba haya salido bien).
- **Sonido atado al mismo consentimiento** (pedido explícito del usuario): nueva `unlockAudioContext()` en `liveAlert.js`, llamada en el mismo clic de "Activar notificaciones" (los dos puntos de entrada: `AuthenticatedLayout.vue` y `PermissionsPrompt.vue`) — ese clic ya es un gesto real de usuario, que es justo lo que el navegador exige para dejar sonar audio sin más restricciones el resto de la sesión.
- **Sonido de arranque reemplazado**: la bocina de barco (dos osciladores graves en diente de sierra) sonaba a alarma de error, no a bienvenida — ahora es un arpeggio corto de tres notas suaves (Do-Mi-Sol, onda senoidal, volumen bajo), mismo criterio sintetizado con Web Audio que el resto de los avisos, sin archivo que alojar.
- **"Instalar app" y "Pasarme a conductor/cliente" también en el perfil** (antes solo en el menú desplegable del header): nueva tarjeta "Cuenta" en `Profile/Edit.vue`, y el mismo bloque en `Driver/Profile.vue` — mismas funciones que ya existían, solo un segundo lugar desde donde usarlas.
- **Verificación de teléfono accesible para el cliente**: el flujo ya existía (`PhoneVerificationController`, aplica a cualquier rol, no solo conductor) pero era 100% reactivo — solo se topaba con él si el middleware lo forzaba al entrar a `/dashboard` con el teléfono sin verificar. Ahora hay un enlace "Verificar ahora" en la tarjeta "Cuenta" del perfil, visible si el teléfono todavía no está verificado — sin tocar nada del backend, la pantalla `Auth/VerifyPhone.vue` y sus rutas ya estaban listas.
- **Bug real: cuentas de Google no podían cambiar su contraseña.** `GoogleAuthController` le arma una contraseña al azar a toda cuenta creada por Google (la columna es obligatoria, pero nadie la conoce nunca) — el formulario de "Cambiar contraseña" de siempre exigía esa contraseña actual para poder cambiarla, algo que esas cuentas jamás iban a poder escribir bien, dejándolas bloqueadas para siempre. Nueva columna `users.password_set_at` (nullable): `null` significa "todavía tiene la del azar de Google, nunca puso una propia" — el formulario pasa a "Crear contraseña" sin pedir la actual. No-nulo significa "ya tiene una propia" (de siempre, o una que ya creó por acá) — sigue pidiendo la actual para cambiarla, como corresponde. Se marca también al registrarse normal y al resetear por "Olvidé mi contraseña" (`NewPasswordController`). **Backfill en la propia migración**: cuentas existentes sin `google_id` se marcan con su fecha de alta (ya tenían una propia); las que sí tienen `google_id` quedan en `null` a propósito — con el flujo viejo, ninguna pudo haber cambiado su contraseña de verdad (el chequeo de "actual" se lo impedía siempre), así que siguen sin una propia hasta que la creen con el flujo nuevo.
- **Usuario y código de socio visibles en el perfil** (pedido explícito del usuario, aclarado en el chat: el código de socio corto —el del buscador— es distinto del código de invitación largo que ya se mostraba): agregado a `Profile/Edit.vue` y `Driver/Profile.vue`, etiquetado para no confundir los dos códigos. No hizo falta tocar ningún controller — `username`/`member_code` ya viajaban sin ocultar en `auth.user` para toda la app.

### Tests
`tests/Feature/Auth/PasswordUpdateTest.php` (+2): una cuenta de Google sin contraseña propia puede crear una sin que le pidan la actual; una que ya tiene una propia (de Google o no) sigue teniendo que darla para cambiarla de nuevo. `tests/Feature/Auth/RegistrationTest.php` (+1): registrarse normal marca `password_set_at`. Nada de PHP para el bug de notificaciones/sonido ni los botones de perfil (cambios puramente de Vue). Suite completa: 686 tests OK, Pint limpio, build limpio.

**Pendiente de tu parte**: la migración nueva (`password_set_at`, con su backfill incluido) tampoco se pudo correr en la base local — mismo MySQL caído, corré `php artisan migrate` apenas lo levantes.
