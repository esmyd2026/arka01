    ...
    public static function forCooperative(
        ...
        return $cooperative->activeDriverMemberships()
            ...
            ->filter(function (User $driver) use ($busyDriverIds, $originLat, $originLng, $passengerCount, $needsTrunk) {
                ...
                // Corrección explícita del usuario sobre una regla anterior
                // ("tiene que tener el plan mayor al gratis"): la cooperativa
                // es quien paga para que su conductor opere dentro de ella —
                // el conductor no paga dos veces. Su plan personal (incluido
                // Gratis) solo sigue limitando su CARTERA PRIVADA propia
                // (ver DriverAccessResolver::privateClientCapacity()), nunca
                // su elegibilidad para el despacho de la cooperativa.
                return $profile
